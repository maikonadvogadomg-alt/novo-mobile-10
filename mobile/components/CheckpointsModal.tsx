      ...
      <View style={styles.overlay}>
        <View style={[styles.sheet, { backgroundColor: colors.background, borderTopColor: colors.border }]}>
          <View style={[styles.header, { borderBottomColor: colors.border }]}>
            <Text style={[styles.title, { color: colors.foreground }]}>Ã°ÂÂÂ¸ Checkpoints do Projeto</Text>
            ...
            <>
              <View style={[styles.saveBox, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <Text style={{ color: colors.foreground, fontWeight: "600", marginBottom: 8 }}>
                  Ã°ÂÂÂ Salvar ponto atual Ã¢ÂÂ {activeProject.name}
                </Text>
                <Text style={{ color: colors.mutedForeground, fontSize: 11, marginBottom: 10 }}>
                  {activeProject.files.length} arquivo(s) ÃÂ· {checkpoints.length}/10 checkpoints salvos
                </Text>
                <View style={{ flexDirection: "row", gap: 8 }}>
                  <TextInput
                    ...
                    placeholder="DescriÃÂ§ÃÂ£o (ex: Antes de refatorar)"
                ...
                {checkpoints.length >= 10 && (
                  <Text style={{ color: "#f59e0b", fontSize: 11, marginTop: 6 }}>
                    Ã¢ÂÂ Ã¯Â¸Â MÃÂ¡ximo de 10 checkpoints. Apague algum para criar novo.
                  </Text>
                )}
              </View>
                ...
                {checkpoints.length === 0 && (
                  <View style={{ alignItems: "center", paddingVertical: 32 }}>
                    ...
                    <Text style={{ color: colors.mutedForeground, fontSize: 12, marginTop: 4 }}>
                      Salve antes de fazer grandes alteraÃÂ§ÃÂµes.
                    </Text>
                  </View>
                )}
                  ...
                  <View key={cp.id} style={[styles.cpRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={{ color: colors.mutedForeground, fontSize: 11, marginTop: 2 }}>
                        {cp.files.length} arquivo(s) ÃÂ· {new Date(cp.createdAt).toLocaleString("pt-BR")}
                      </Text>
                    </View>
