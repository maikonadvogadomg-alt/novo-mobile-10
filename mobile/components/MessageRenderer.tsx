    ...
    >
      ...
      <View style={[styles.codeHeader, { borderBottomColor: colors.border }]}>
        <View style={styles.codeLangRow}>
          ...
          <Text style={styles.codeLang}>{language || "cÃÂ³digo"}</Text>
        </View>
  ...
  const handleApplyToFile = (code: string) => {
    if (!activeProject || !activeFile) {
      Alert.alert(
        ...
        "Abra um arquivo no Editor antes de aplicar o cÃÂ³digo.",
        [{ text: "OK" }]
      );
      return;
    }
    Alert.alert(
      "Aplicar ao arquivo",
      `Isso vai substituir o conteÃÂºdo de "${activeFile.name}". Continuar?`,
      [
        ...
        if (seg.type === "code") {
          return (
            <CodeBlock
              key={i}
              code={seg.content}
              language={seg.language || "cÃÂ³digo"}
              onApplyToFile={showApply && !isUser ? handleApplyToFile : undefined}
            />
          );
        }
        // Plain text segment Ã¢ÂÂ clean up leading/trailing blank lines
