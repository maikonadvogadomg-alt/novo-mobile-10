...
const CATEGORIES: { key: AIMemoryEntry["category"]; label: string; color: string }[] = [
  { key: "usuario", label: "Ã°ÂÂÂ¤ UsuÃÂ¡rio", color: "#6366f1" },
  { key: "projeto", label: "Ã°ÂÂÂ Projeto", color: "#00d4aa" },
  { key: "preferencia", label: "Ã¢ÂÂÃ¯Â¸Â PreferÃÂªncia", color: "#f59e0b" },
  { key: "geral", label: "Ã°ÂÂÂ¡ Geral", color: "#888" },
];
...
export default function AIMemoryModal({
  ...
  const handleRemove = (id: string) => {
    if (Platform.OS === "web") {
      ...
      Alert.alert("Remover memÃÂ³ria", "Apagar este registro?", [
  ...
  const handleClearAll = () => {
    if (Platform.OS === "web") {
      ...
      Alert.alert("Limpar memÃÂ³ria", "Apagar TODA a memÃÂ³ria da IA?", [
        { text: "Cancelar", style: "cancel" },
      ...
      <View style={styles.overlay}>
        <View style={[styles.sheet, { backgroundColor: colors.background, borderTopColor: colors.border }]}>
          <View style={[styles.header, { borderBottomColor: colors.border }]}>
            <Text style={[styles.title, { color: colors.foreground }]}>Ã°ÂÂ§Â  MemÃÂ³ria da IA</Text>
          ...
          <View style={[styles.infoBox, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Text style={{ color: colors.mutedForeground, fontSize: 12 }}>
              ...
              {" ÃÂ· "}{aiMemory.length}/50 registros
            </Text>
          </View>
              ...
              >
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={{ color: colors.mutedForeground, fontSize: 10, marginTop: 4 }}>
                    {CATEGORIES.find((c) => c.key === entry.category)?.label} ÃÂ· {new Date(entry.createdAt).toLocaleDateString("pt-BR")}
                  </Text>
                </View>
          ...
          <View style={[styles.addBar, { borderTopColor: colors.border, backgroundColor: colors.card }]}>
            ...
            <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
              <TextInput
                ...
                placeholder="Adicionar memÃÂ³ria... (ex: Prefiro TypeScript)"
